...
public class AssetPairsDBDao extends AbstractDBDao implements IAssetPairsDao {
	...
	private static final String Q_GET_VALIDS_NO_DOT_D = "SELECT * FROM ASSET_PAIRS WHERE EXPIRE_TIME = 0 AND PAIR_NAME NOT LIKE '%.d'";
	...
	@Override
	public void persistAssetPairs(Long callTime, List<AssetPair> newAssetPairs) {
		int numUpdated = super.performUpdate(Q_UPDATE_EXPIRE_TIME, callTime);
		logger.fine("Updated expire time for %d asset pairs (from 0 to %d)", numUpdated, callTime);

		String strVal = Utils.join(newAssetPairs, ",", ap -> assetPairToValues(callTime, ap));
		String query = Q_INSERT_NEW.replace(PH_PAIRS_VALUES, strVal);
		int numInsert = super.performUpdate(query);
		logger.fine("Insert %d new asset pairs (callTime=%d)", numInsert, callTime);

		// Read just insert records to retrieve pairIDs (autogenerated by DB)
		Map<Long, AssetPair> apMap = inquiryTableAssetPairs(false);
	...
	private Map<Long, AssetPair> inquiryTableAssetPairs(boolean discardDotD) {
		String query = discardDotD ? Q_GET_VALIDS_NO_DOT_D : Q_GET_VALIDS;
	...
	private void insertFeesAndLeverage(Map<Long, AssetPair> apMap, List<AssetPair> newAssetPairs) {
		...
		if(sbFee.length() > 0) {
			String query = Q_INSERT_FEE.replace(PH_FEE_VALUES, sbFee.toString());
			int numInsert = super.performUpdate(query);
			logger.finer("Insert %d asset pairs fees", numInsert);
		}

		if(sbLeverage.length() > 0) {
			String query = Q_INSERT_LEVERAGE.replace(PH_LEVERAGE_VALUES, sbLeverage.toString());
			int numInsert = super.performUpdate(query);
			logger.finer("Insert %d asset pairs leverages", numInsert);
		}
	}
